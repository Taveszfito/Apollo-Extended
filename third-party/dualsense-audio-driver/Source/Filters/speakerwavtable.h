...
//
#define SPEAKER_DEVICE_MAX_CHANNELS         4    // DualSense speaker + haptic actuators.

#define SPEAKER_HOST_MAX_CHANNELS           4    // DualSense USB audio exposes four channels.
...
{
    //-------------------------------------------------
    // 0) 16-bit, Quad, 48 kHz (DualSense native USB audio)
    //-------------------------------------------------
    {
        {
...
{
    {
        ...
        1,  // DualSense exposes a fixed 48 kHz, four-channel, 16-bit stream.
        SpeakerHostPinSupportedDeviceModes,
        6  // Raw, default, media, movie, communications, and notification.
    },
    {
        BridgePin,
        NULL,
...
// Keep the advertised data range in sync with the only exposed host format:
// 4-channel, 16-bit PCM at 48 kHz.
//
static
