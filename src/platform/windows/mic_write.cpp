...
namespace platf::audio {
  ...
  int mic_write_wasapi_t::init() {
    ...
    // Find the Steam Streaming Microphone render endpoint, or the current default
    // speakers when this instance is used as a troubleshooting monitor.
    ...
    if (!use_default_render_device && !switch_default_capture()) {
      BOOST_LOG(warning) << "[mic] Steam Streaming Microphone capture endpoint was not made the Windows default"sv;
    }

    return 0;
  }

  bool mic_write_wasapi_t::switch_default_capture() {
    IMMDevice *raw_previous = nullptr;
    if (SUCCEEDED(device_enum->GetDefaultAudioEndpoint(eCapture, eConsole, &raw_previous)) && raw_previous) {
      util::safe_ptr<IMMDevice, release_com<IMMDevice>> previous(raw_previous);
      WCHAR *id = nullptr;
      if (SUCCEEDED(previous->GetId(&id)) && id) {
        previous_capture_id = id;
        CoTaskMemFree(id);
      }
    }

    IMMDeviceCollection *raw_collection = nullptr;
    if (FAILED(device_enum->EnumAudioEndpoints(eCapture, DEVICE_STATEMASK_ALL, &raw_collection)) || !raw_collection) {
      return false;
    }
    ...
    for (UINT i = 0; i < count; ++i) {
      ...
      if (FAILED(prop->GetValue(local_PKEY_Device_FriendlyName, &value)) || value.vt != VT_LPWSTR || !value.pwszVal) {
      ...
      if (name.find(L"steam streaming microphone") == std::wstring::npos) continue;
      WCHAR *id = nullptr;
    ...
    if (steam_capture_id.empty()) return false;

    IPolicyConfig *raw_policy = nullptr;
    if (FAILED(CoCreateInstance(CLSID_CPolicyConfigClient, nullptr, CLSCTX_ALL,
                                IID_IPolicyConfig, (void **)&raw_policy)) || !raw_policy) return false;
    util::safe_ptr<IPolicyConfig, release_com<IPolicyConfig>> policy(raw_policy);
    policy->SetEndpointVisibility(steam_capture_id.c_str(), TRUE);
    bool ok = true;
    for (ERole role : {eConsole, eMultimedia, eCommunications}) {
      if (FAILED(policy->SetDefaultEndpoint(steam_capture_id.c_str(), role))) ok = false;
    }
    if (ok) BOOST_LOG(info) << "[mic] Windows default microphone switched to Steam Streaming Microphone"sv;
    return ok;
  }

  void mic_write_wasapi_t::restore_default_capture() {
    if (previous_capture_id.empty() || previous_capture_id == steam_capture_id) return;
    IPolicyConfig *raw_policy = nullptr;
    if (FAILED(CoCreateInstance(CLSID_CPolicyConfigClient, nullptr, CLSCTX_ALL,
                                IID_IPolicyConfig, (void **)&raw_policy)) || !raw_policy) return;
    util::safe_ptr<IPolicyConfig, release_com<IPolicyConfig>> policy(raw_policy);
    for (ERole role : {eConsole, eMultimedia, eCommunications}) {
      policy->SetDefaultEndpoint(previous_capture_id.c_str(), role);
    }
    BOOST_LOG(info) << "[mic] Restored previous Windows default microphone"sv;
  }

  bool mic_write_wasapi_t::find_target_device(std::wstring &out_device_id) {
    if (use_default_render_device) {
      IMMDevice *raw_device = nullptr;
      if (FAILED(device_enum->GetDefaultAudioEndpoint(eRender, eConsole, &raw_device)) || !raw_device) {
        BOOST_LOG(warning) << "[mic-monitor] Windows default render endpoint not found"sv;
        return false;
      }
      util::safe_ptr<IMMDevice, release_com<IMMDevice>> device(raw_device);
      WCHAR *raw_id = nullptr;
      if (FAILED(device->GetId(&raw_id)) || !raw_id) return false;
      out_device_id = raw_id;
      CoTaskMemFree(raw_id);
      target_device_name = "Windows default speakers";
      return true;
    }
  ...
  bool mic_write_wasapi_t::initialize_device(const std::wstring &device_id) {
    ...
    hr = audio_client->Initialize(AUDCLNT_SHAREMODE_SHARED,
      AUDCLNT_STREAMFLAGS_EVENTCALLBACK | AUDCLNT_STREAMFLAGS_AUTOCONVERTPCM |
        AUDCLNT_STREAMFLAGS_SRC_DEFAULT_QUALITY, buffer_duration, 0,
  ...
  int mic_write_wasapi_t::write_pcm(const float *samples, std::uint32_t frame_count) {
    ...
    if (!first_packet_written_logged) {
      ...
      BOOST_LOG(info) << (use_default_render_device ? "[mic-monitor] first PCM write to default speakers"sv
                                                         : "[mic] first PCM write to Steam mic backend"sv);
    }
